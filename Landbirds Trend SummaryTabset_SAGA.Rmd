...
<ul>
<li> At `r params$Park`, a total of `r NCRNbirds::birdRichness(BirdData[ParkList == params$Park])` bird species have been detected between `r params$minYear` to `r params$maxYear` with an average of `r NCRNbirds::birdRichness(BirdData[ParkList == params$Park], byYear = TRUE) %>% dplyr::pull(Richness) %>% mean() %>% round(0)` species detected per year (Figure 1, blue). It is likely that not all species encountered during monitoring breed within the park but the number breeding is close to the observed average species richness. Total pecies richness at `r params$Park` ranks as #5 among the 10 NETN parks monitored (Figure 2).</li>
<p>
<li> The number of species has remained <b> unchanged </b> between `r params$minYear` to `r params$maxYear` in `r params$Park` (Figures 1 and 3). It is important to note that the number of <i>estimated</i> species as shown in Figure 1 (green) is typically higher than the actual number detected during monitoring (blue) because not all birds may have been detected during the survey due to imperfect detection (e.g., bird is present but not heard or seen). This was only the case, however, in 2014 and 2019. At `r params$Park` the model actually estimated a lower annual species richness than what was observed because it also accounted for local forest structure (see Doser et al. 2021). The best explanation for why estimated species richness is <b>lower than</b> observed richness in most years is because the bird diversity at `r params$Park` is on average greater than in other network parks with similar forest structure as measured by the regeneration layer, forest cover, and live tree basal area. </li> 
<p>
<li> While the number of forest bird species in `r params$Park` has not changed since monitoring began, it is one of 4 `r params$network` parks showing a slight (but non-significant) decline in estimated species richness over time (Figure 3). </li>
</ul>
...
<p> While no formal analysis has been completed, this map indicates that within the park more species have been detected along the eastern edge of Blow-me-down Pond. </p>
...
Figure 4 shows that the BCI rating at `r params$Park` has remained relatively unchanged since `r params$minYear` and is of "High Integrity" compared to other bird communities within the Central Appalachian region. 
...
Bird Community Index ratings for permanent monitoring sites from `r params$minYear` to `r params$maxYear` are shown below across `r params$Park`. BCI scores are fairly uniform across the park with 62.5% of sites characterized as High Integrity (5 of 8 sites) and 3 of 8 (37.5%) Highest Integrity compared to other forests in the Central Appalachian region.  While most of these sites are located near the park boundary, they were not necessarily near a forest edge. Overall, this is excellent news and means that specialist forest birds have been breeding across the park.
...
As of `r params$maxYear`, `r params$Park` has the <strong>3rd</strong> highest BCI rating in `r params$network` (Figure 5). The trend analysis concluded that trends in overall bird abundance across the network are declining the fastest in parks with the highest BCI rating (Doser et al. 2021) (Figure 6).   
...
<strong> Eleven songbird species of regional conservation concern within the `r params$BCRName` BCR have been detected at `r params$Park` </strong>, including Black-and-white Warbler, Black-throated Blue Warbler, Eastern Wood-Pewee, Least Flycatcher, Northern Flicker, Northern Parula, Rose-breasted Grosbeak, Scarlet Tanager, Veery, Wood Thrush, and Yellow-throated Vireo. Of these, 4 are among the 10 most abundant species detected (Figure 7). </p>
...
<p>None of the 10 most abundant songbirds detected in `r params$Park` have increased significantly over time since `r params$minYear`. Species of regional conservation importance (denoted by *), such as Eastern Wood-Peewee, was estimated as having a slightly positive trend whereas Scarlet Tanager, Wood Thrush, and Veery appear to be in decline, but not significantly. Ovenbird is the only common species that has declined significantly from `r params$minYear` to `r params$maxYear`. </p>
<p>
...
Comparisons of NETN monitoring data in `r params$Park` to the results from the BBS suggest that trends in park-scale abundance are generally consistent with trends at the `r params$BCRName` BCR-scale (8 of 10 species) and less so at the state-scale (4 of 10). The top 4 species encountered at `r params$Park` are in decline at the park and in the surrounding `r params$BCRName` BCR with the exception of Ovenbird, which has been stable and showing a slight increase in abundance across the `r params$BCRName` BCR and `r params$state` since 2006. 

Observed declines of Wood Thrush, Scarlet Tanager, and Eastern Towhee -- species of regional conservation importance -- at `r params$Park` are concerning and also consistent with the broader patterns observed in the `r params$BCRName` BCR. However, besides Scarlet Tanager, which was estimated as declining significantly at `r params$Park`, Eastern Towhee has not changed significantly within this time period in all areas evaluated. Additionally, while Wood Thrush is the most abundant species detected at the park but in decline, it is significantly declining across the `r params$BCRName` BCR. Eastern Wood-Pewee is the only species of regional conservation importance with stable populations at the park and within the broader region. Finally, American Robin is the only species that has declined significantly since 2006 across park, state and BCR scales. 
...
options(knitr.kable.NA = '')# remove NAs from table
...
```{r concl, child = "sections\\SAGA_conclusions.Rmd", echo=FALSE, message=FALSE, warning = FALSE}
