...
kableExtra::kbl(TableData, escape = FALSE, align= "l", caption = paste0("Comparison of park trends to regional trends from the USGS Breeding Bird Survey (BBS; Sauer et al. 2020) for the most commonly detected species at ", params$Park, ". An * next to a species name denotes species designated by Partners in Flight to be of conservation importance within the ", params$BCRName, " BCR (Panjabi et al. 2020). ** denotes a statistically significant Increase or Decrease in bird abunce abundance over time; n.s. = non-significant. Note that we were unable to access BBS trends for some species."))%>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive")) 
```

Comparisons of NETN monitoring data to the results from the BBS indicate that trends in park-scale abundance of the 10 most commonly encountered species are generally consistent with BBS results at the state (8 of 10 species) and BCR-scales (7 of 10 species). Unfortunately, most of the species in decline at the  park are also in decline at the state and BCR scales except for Veery. On the other hand, Red-eyed Vireo, Eastern Wood-Pewee, and Black-capped Chickadee appear to be stable but increasing at `r params$Park`  and most scales. Ovenbird, which has declined at `r params$Park` since ` r params$MinYear` is also declining in NH but its populations are increasing in the ` r params$BCRName` BCR. 
...
BBS_tlu<- BBS_trends %>% distinct(AOU_Code, common, Region, Region.Name) %>% 
...
plotdata.netn<-NETNplotdata %>% filter(variable == "Trend Estimate") %>% 
  dplyr::select(`Common Name`, Year, Index = value, valueCenter) %>% 
  
  add_column(Region.Name = params$Park) 
  


plotdata<- dplyr::select(plotdata.bbs,`Common Name`,Region.Name, Year, Index, valueCenter) %>% 
  bind_rows(plotdata.netn)

GraphOut<-ggplot(data=plotdata, aes(x = Year, y = Index, color = Region.Name, group = Region.Name))+expand_limits(y=0)+
