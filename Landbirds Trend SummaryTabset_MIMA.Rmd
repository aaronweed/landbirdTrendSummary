...
NCRNbirds::mapEffort(BirdData[ParkList == params$Park], points = forest.sites, palette = "viridis", title = paste0("Total surveys since ",params$minYear))
```
<p class='capwrap'>
Map of monitoring effort (no. surveys) at `r params$Park`'s 23 permanent forest and 6 grassland sites between `r params$minYear` and `r params$maxYear`. Clicking on the site displays the unique site identifier and total number of surveys since `r params$minYear`. Grassland sites are located along the Battle Road Trail in the central and western part of the park. Data from those sites are not included in this report.</p>
</div>
...
Please note that bird monitoring at MIMA includes a combination of forested (23) and grassland (6) sites. The contents of this report primarily focus on data summaries from the forested sites unless otherwise denoted. However, given the proximity of the grassland sites (blue dots labeled as MIMA39## in the above map) to the forested sites there was a lot of overlap in species that were detected in both habitats. For instance, of the 76 species detected at MIMA only 9 were only detected at the grassland sites and 7 unque to the forested sites.
...
<ul>
<li> At `r params$Park`, a total of `r NCRNbirds::birdRichness(BirdData[ParkList == params$Park])` bird species have been detected between `r params$minYear` to `r params$maxYear` with an average of `r NCRNbirds::birdRichness(BirdData[ParkList == params$Park], byYear = TRUE) %>% dplyr::pull(Richness) %>% mean() %>% round(0)` species detected per year (Figure 1, blue).  Species richness at `r params$Park` ranks as #3 among the 10 NETN parks included in this report (Figure 2).</li>
<p>
<li> The number of species has remained <b> unchanged </b> between `r params$minYear` to `r params$maxYear` in `r params$Park` (Figures 1 and 3). It is important to note that in any given year the number of <i>estimated</i> species as shown in Figure 1 (green) is typically higher than the actual number detected during monitoring (blue) because not all birds may have been detected during the survey due to imperfect detection (e.g., bird is present but not heard or seen). The trend model (Figure 1, green) estimated fewer species in 2017 and 2019 but overall the species diversity detected during monitoring is very similar from year to year and values fall within the confidence intervals (gray shading). </li> 
<p>
<li> While the number of forest bird species in `r params$Park` has not changed, it is one of 4 `r params$network` parks showing a slight (but non-significant) increase in estimated species richness over time (Figure 3). </li>
</ul>
...
plotdata<- dplyr::filter(ParkCommAnnual, park == params$Park) %>% select(Year= year, `Estimated Richness` = median.rich, low = low.95.rich, hi = high.95.rich) %>% 
  left_join(., NCRNbirds::birdRichness(BirdData[ParkList == params$Park], band= 1:2, byYear = TRUE, points = forest.sites), by = "Year") %>% dplyr::rename(`Observed Richness` = Richness) 
...
mapRichness(BirdData[ParkList == params$Park], title = "No. of species", palette = "viridis", band = 1:2, points = forest.sites)
```
<p class='capwrap'>
