...
# exclude grassland sites from the queries by selecting the forest sites
...
sp_detects<- SumRelAbund(BirdData[ParkList == params$Park], band = 1:2, max= T, points = forest.sites) %>% # raw detection data created from SumRelAbund
...
NCRNbirds::mapEffort(BirdData[ParkList == params$Park], points = forest.sites, palette = "viridis", title = paste0("Total surveys since ",params$minYear)) %>% addPolygons(sara_veg)

```
<p class='capwrap'>
Map of monitoring effort (no. surveys) at `r params$Park`'s 34 permanent forest and 6 grassland sites between `r params$minYear` and `r params$maxYear`. Clicking on the site displays the unique site identifier and total number of surveys since `r params$minYear`. Grassland sites are located along the Battle Road Trail in the central and western part of the park. Data from those sites are not included in this report.</p>
</div>
...
<ul>
 <li> <b>Bird Diversity:</b> A summary of the number of species detected since `r params$minYear` across all monitoring sites located in grassland and forestede habiats.</li> 
 <li> <b>Bird Community Index:</b> A summary of the condition of the park's <b> forest </b> bird community.</li> 
 <li> <b>Species Trends:</b> A summary of bird abundance over time for the most commonly detected species across our forest sites. </li> 
 <li> <b>Conclusions</b>: A summary of forest bird monitoring results in the context of park forest health. </li>
 <li> <b>Supporting Resources:</b> List of references and associated documentation for the bird monitoring program.</li> 
 </ul>

Bird monitoring at `r params$Park` includes a combination of sites located in areas that are primarily forested (34) or within open, grassland (25) habitats. Given the proximity of the sites to one another many species are detected at sites located in both habitats. For instance, of the 101 species detected, only 13 species have only been detected in the grassland sites and 22 are unique to the forested sites. Common Yellowthroat, Red-winged Blackbird, and Black-capped Chickadee are examples of species found at sites within both habitats whereas Bobolink, Eastern Meadowlark, Savannah Sparrow and Eastern Bluebird are found primarily in open habitats. 

In this report summaries of species diversity (richness) include data from all monitoring sites whereas as the Bird Community Index and analyses involving species abundance trends <u> only </u> consider monitoring data from sites located in forested habitat. The reason for this is because the Bird Community Index is derived strictly for measuring the health of the forest bird community and the analysis of abundance utilized NETN's forest vegetation monitoring data to evaluate effects of forest structure on the bird community. We lack similar vegetation data near our grassland monitoring sites. A more specific analysis of the grassland bird community is forthcoming.   
...
<p>  The number of detected species (or species richness) is a key metric from our monitoring because it provides a straight-forward indication of the bird diversity using `r params$Park` as breeding habitat during the summer. The number of species may change over time due to many factors affecting the local area surrounding the park (e.g., weather, forest cover, understory vegetation) or factors affecting bird populations at greater geographic scales, such as trends in regional forest structure (Holmes and Sherry 2001) and wintering habitat survival. The following summary was derived from sites within the park's forest and open, grassland sites.</p> 
...
<ul>
<li> At `r params$Park`, a total of `r NCRNbirds::birdRichness(BirdData[ParkList == params$Park])` bird species have been detected between `r params$minYear` to `r params$maxYear` with an average of `r NCRNbirds::birdRichness(BirdData[ParkList == params$Park], byYear = TRUE) %>% dplyr::pull(Richness) %>% mean() %>% round(0)` species detected per year across forested and grassland sites (Figure 1, blue). Species richness at our `r params$Park` sites ranks as #1 among the 10 NETN parks included in the analysis (Figure 2)!</li>
<p>
<li> Although slightly increasing over time, the number of species has remained <b> unchanged </b> (statistically speaking) between `r params$minYear` to `r params$maxYear` in `r params$Park` (Figures 1 and 3). It is important to note that in any given year the number of <i>estimated</i> species as shown in Figure 1 (green) maybe higher than the actual number detected during monitoring (blue) because not all birds may have been detected during the survey due to imperfect detection (e.g., bird is present but not heard or seen). The trend model (Figure 1, green) estimated fewer species in a couple of years but overall the species diversity detected during monitoring is very similar from year to year and most field observations fall within the confidence intervals (gray shading). </li> 
<p>
<li> While the number of forest bird species in `r params$Park` has not changed, it is one of 4 `r params$network` parks showing a slight (but non-significant) increase in species richness over time (Figure 3). </li>
</ul>
...
```
<p class='capwrap'>
Map of cumulative bird species richness detected at `r params$Park`'s permanent forest monitoring sites between `r params$minYear` and `r params$maxYear`. Click on the sites to see species richness observed per site.</p>
</div>

<p> While no formal analysis has been completed, this map shows that cumulative species richness varies across the monitoring sites with a slight increase in species diversity detected in the NW portion of the monitoring area. Note that these differences among sites may be due to habitat quality but maybe also be due to differences in the number of surveys at our sites since monitoring began. </p>
...
map_figcap <- paste0("Map of Bird Community Index ratings for permanent forest monitoring sites in ", params$Park, " from ",  params$minYear, " to ", params$maxYear, ". Data summaries exclude species not suspected to be breeding in the forest (e.g., flyovers). Clicking on the site displays the unique site name and the BCI rating.")
```
...
PIFSpecies<- getChecklist(BirdData[ParkList == params$Park], out.style = "AOU", band=1:2, points = forest.sites) %>% 
...
aou<-SumRelAbund(BirdData[ParkList == params$Park], max=T, sort=TRUE, abund = 20, band = 1:2, points = forest.sites)$AOU_Code # get Top 20 most abundant species (birds/point)
...
aou<-SumRelAbund(BirdData[ParkList == params$Park], max=T, sort=TRUE, abund = 10, band = 1:2, points = forest.sites)$AOU_Code # get Top 10 most abundant species (birds/point)
...
<p> In parks with significant decreasing or increasing trends, most species generally showed a similar trend to the overall park trend, but often only a fraction of the species-specific trends were significantly different from zero. For example, while the trend analysis indicated that  28 species are in decline at `r params$Park`, only 1 has declined significantly since `r params$minYear` (Figures 8 and 9c). It is important to acknowledge declines in relative abundance even for species with trends that were non-significant in our study. This is because changes in the abundance of rare or less commonly detected species are usually less likely to show statistically significant trends in monitoring data yet are of conservation interest. Hence, lack of statistical significance does not mean that we shouldn't be concerned about species we are seeing fewer of over time -- our methods may not be sensitive enough to detect statistically significant changes. On other hand, perhaps the most alarming result from the analysis is that birders have observed fewer of the most abundant species at `r params$Park` over time (see "Plots of Species Abundance"). </p>
...
#Get Top 10 species
aou<-SumRelAbund(BirdData[ParkList == params$Park], max=T, sort=TRUE, abund = 10, band = 1:2,points = forest.sites)$AOU_Code
...
aou<-SumRelAbund(BirdData[ParkList == params$Park], max=T, sort=TRUE, abund = 10, band = 1:2,points = forest.sites)$AOU_Code # get Top 20 most abundant species (birds/point)
...
Comparisons of NETN monitoring data for the top 10 most abundant species in `r params$Park` to the results available from the BBS suggest that trends in park-scale abundance are somewhat consistent with trends at the `r params$BCRName` BCR-scale and state-scale (4 of 8 species). BBS trends of these species at the `r params$BCRName` BCR and state-scale are identical with the exception of North Cardinal and White-breasted Nuthatch, which have increased significantly within the `r params$BCRName` BCR but not in Massachusetts. Declining trends in abundance, regardless of whether they are significant or not, of Blue Jay, American Robin, American Crow, and Black-capped Chickadee are consistent across `r params$Park` and BCR- and state-scales as is the non-significant increase in Gray Catbirds. Decreases observed in Northern Cardinal and White-breasted Nuthatch at the park but not at the other scales is notable but our monitoring data suggests that both species have stable breeding populations at the park. 
