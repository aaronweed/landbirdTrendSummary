...
##### import data estimated from Doser et al. 2021----

#The files "spMeanTrends.csv" and "spMeanYearEffect.csv" contains the model estimates from Doser et al. 2021. spMeanYearEffect.csv contain the estimated change in abundance every four years on the log-scale after accounting for imperfect detection and effects of forest covariates. spMeanTrends.csv contain the annual estimates from the trend. The trend is for every four years as a result of standardizing the year variable in the model (which was necessary to get the model to converge with the other forest covariates in the model). It turns out the standard deviation of the year covariate was right around four (3.98), hence the interpretation of the trend parameter for every four years. 
...
guildsMeanTrends <-read.csv("./Data/guildsMeanTrends.csv") # guild-level 4-year log model estimates

pointCommunity <- read.csv("./Data/point-rich-bci-by-year.csv")
 
ParkCommAnnual <- read.csv("./Data/park-rich-bci-by-year.csv")

##### Import and create some look-ups ----
...
##### Derive relative abundance and add on Doser's modeled estimates ----
...
# Define min and max years from monitoring
...
```{r Effmap, echo = FALSE,  fig.height=6, fig.width= 6, fig.align = 'left', fig.cap = "Map of monitoring effort at permanent sites", warning= FALSE, comment=FALSE, message=FALSE}
...
Figure 4 shows that the BCI rating at `r Park` has remained relatively unchanged since `r minYear` and is of "High Integrity' compared to other bird communities within the Central Appalachian region. 
...
As of `r maxYear`, `r Park` has the 4th highest BCI rating in `r network` (Figure 5). The trend analysis concluded that trends in overall bird abundance are declining the fastest in parks with the highest BCI rating (Doser et al. 2021) (Figure 6).   
...
plotdata<-filter(spMeanYearEffect, AOU_Code %in% aou) %>% 
   left_join(spp,., by = "AOU_Code") %>% 
    mutate(common = forcats::fct_reorder(common, order)) %>% 
    mutate(common = paste0(common, " (", order,")")) %>% 
  left_join(.,PIFSppByBCR, by= "AOU_Code") %>% # add PIF Designations
  mutate(PIF=  if_else(Regional.Importance == 1,"*","")) %>% 
  mutate(common2 = if_else(is.na(PIF), common, paste(PIF,"",common))) %>% 
  mutate(common2 = forcats::fct_reorder(common2, mean)) %>% 
  mutate(mean_Per = paste(round((1-exp(mean))*100),"%")) # calc 1-exp(trend) to determine % change every 4 years
