...
GraphOut<-ggplot(data=plotdata, aes(x = Year, y = value, color = variable))+expand_limits(y=0)+
     ...
     #geom_line(data = filter(plotdata, Nonsig == FALSE & variable == "Trend Estimate")) +
      ...
      geom_smooth(data= filter(plotdata, Nonsig == FALSE & variable == "Trend Estimate"), method = "lm", se= F)

GraphOut+
  geom_text(data= dplyr::filter(plotdata, variable == "Trend Estimate")  %>% distinct(common, park, variable, mean, lowCI, highCI, per_change), show.legend = FALSE, aes(x = Inf, y = Inf, label=  paste(round(mean,3)," (",round(lowCI,2),",",round(highCI,2),")") , hjust = 1.05,vjust   = 1))
...
kableExtra::kbl(TableData, escape = FALSE, align= "c", caption = paste0("Comparison of park trends to regional USGS Breeding Bird Survey data for the most commonly detected species at ", params$Park, ". * denotes a statistically significant increase or decrease in abundance; n.s. = non-significant. Asterisks next to a species name denote species designated by Partners in Flight to be of conservation importance within the ", params$BCRName, " BCR (Panjabi et al. 2020)."))%>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive")) 
```

```{r BBSplots, echo= FALSE, results = 'hide', fig.keep='all', cache= FALSE,  fig.height= 12, fig.width = 8, fig.align = 'center', message=FALSE, warning = FALSE, comment=NA, fig.cap= "Figure 11. Annual index reported by the NA BBS (sauer et al. 2020)"}
...
GraphOut<-ggplot(data=plotdata, aes(x = Year, y = Index, color = Region.Name))+expand_limits(y=0)+
      geom_line(size = 1.5)+
      #geom_ribbon(aes(ymin= Index - lowCI, ymax= Index + hiCI, color= Region), width = .2, color = "grey70")+
      theme_classic()+
      theme(axis.title.y =element_text(size = 14, face ="bold", vjust= 1))+
      theme(axis.title.x =element_text(size = 14, face ="bold", vjust= 1))+
      theme(axis.text.y = element_text(color="black", vjust= 0.5,size = 12))+
      theme(axis.text.x = element_text(color="black", size = 10))+
      scale_x_continuous(breaks = seq(1966, 2019,10)) +
      scale_color_manual(values = c("#2171b5", "black"))+
