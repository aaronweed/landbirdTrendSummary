...
NCRNbirds::mapEffort(BirdData[ParkList == params$Park], palette = "viridis", title = paste0("Total surveys since ",params$minYear)) %>% addPolylines(data= stands, weight=1)
```
<p class='capwrap'>
Map of monitoring effort (no. surveys) at `r params$Park`'s permanent sites between `r params$minYear` and `r params$maxYear`. Clicking on the site displays the unique site identifier and total number of surveys since `r params$minYear`. Blue lines denote forest management stands.</p>
</div>
...
<ul>
<li> At `r params$Park`, a total of `r NCRNbirds::birdRichness(BirdData[ParkList == params$Park])` bird species have been detected between `r params$minYear` to `r params$maxYear` with an average of `r NCRNbirds::birdRichness(BirdData[ParkList == params$Park], byYear = TRUE) %>% dplyr::pull(Richness) %>% mean() %>% round(0)` species detected per year (Figure 1, blue).  Note that at least two species, and possibly others detected, may not be breeding in the park  (e.g., Spotted Sandpiper) or are breeding in non-forested areas surrounding our sites (e.g., Bobolink). Species richness at `r params$Park` ranks as #4 among the NETN parks monitored (Figure 2).</li>
<p>
...
mapRichness(BirdData[ParkList == params$Park], title = "No. of species", palette = "viridis", band = 1:2)%>% addPolylines(data= stands, weight=1)
```
<p class='capwrap'>
Map of cumulative bird species richness detected at `r params$Park`'s permanent monitoring sites between `r params$minYear` and `r params$maxYear`. Click on the sites to see species richness observed per site. Blue lines denote forest management stands.</p>
</div>
...
map_figcap <- paste0("Map of Bird Community Index ratings for permanent monitoring sites in ", params$Park, " from ",  params$minYear, " to ", params$maxYear, ". Data summaries exclude species not suspected to be breeding in the forest (e.g., flyovers). Clicking on the site displays the unique site name and the BCI rating. Blue lines denote forest management stands.")
```

```{r BCIrichmap, echo = TRUE, echo=FALSE, fig.height=6, fig.width= 10, fig.align = 'left', message=FALSE, fig.cap = map_figcap}

mapBCI(BirdData[ParkList == params$Park], band =2, type= "Cent_Appal", years = params$minYear:params$maxYear, maptype = "basic")%>% addPolylines(data= stands, weight=1)
```
